//
//  PortraitCaptureViewController.m
//  BLCardRecognition
//
...
#import "IDFrontScaningView.h"

@interface PortraitCaptureViewController ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
@implementation PortraitCaptureViewController
...
- (void)initView
{
    ...
    IDFrontScaningView *IDCardScaningView = [[IDFrontScaningView alloc] initWithFrame:self.view.frame];
    IDCardScaningView.tag = 1005;
...
-(void)turnOnOrOffTorch {
    ...
    if ([self.device hasTorch]){ // 判断是否有闪光灯
        ...
        IDFrontScaningView *IDCardScaningView = (IDFrontScaningView *)[self.view viewWithTag:1005];
...
- (void)IDCardRecognit:(CVImageBufferRef)imageBuffer {
    ...
    if (CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess) {
        ...
        if (ret <= 0) {
            ...
            IDResultModel *iDInfo = [[IDResultModel alloc] init];
            ...
            while(i < ret){
                ...
                if(xlen) {
                    ...
                    NSString *temp = [NSString stringWithCString:(char *)content encoding:gbkEncoding];
                    NSLog(@" \n temp --- %@",temp);
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                iDInfo.frontImage = subImage;
                
//                // 推出IDInfoVC（展示身份证信息的控制器）
//                IDCardInfoViewController *IDInfoVC = [[IDCardInfoViewController alloc] init];
//
...
//                IDInfoVC.IDImage = subImage;// 身份证图像
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [weakSelf dismissViewControllerAnimated:YES completion:^{
                        if (strongSelf.recognitionHandle) strongSelf.recognitionHandle(iDInfo);
